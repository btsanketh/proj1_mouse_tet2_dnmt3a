...
for (celltype in celltypes) {
  ...
  df_ct$diffregulated[df_ct$log2fc < -0.05 & df_ct$y >= 10] <- "DOWN"
   ...
   #differentially expressed (NA in case they are not)
    ...
    rownames(df_macs[df_ct$diffregulated != "NO",])
    
    #Make the volcano plot
    plot <- ggplot(data=df_ct, aes(x=log2fc, y= y, col=diffregulated,
                         label = delabel))+ 
    geom_point() + 
    theme_minimal()+
    geom_text() +
...
for (celltype in celltypes) {
  ...
  df_ct$diffregulated <- "NO"
  # if log2Foldchange > 0.6 and pvalue < 0.05, set as "UP" 
  df_ct$diffregulated[df_ct$log2fc > 0.05 & df_ct$y >= 4] <- "UP"
  # if log2Foldchange < -0.6 and pvalue < 0.05, set as "DOWN"
  df_ct$diffregulated[df_ct$log2fc < -0.05 & df_ct$y >= 4] <- "DOWN"
